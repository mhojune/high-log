...
export const SettingBox = styled.div`
  ...
  align-items: flex-start;
  border-radius: 12px;
  border: 1px solid ${({ theme }) => theme.colors.secondary["07"] || "#D4D9F9"};
  background: ${({ theme }) => theme.colors.secondary["09"] || "#F6F6FB"};
  gap: 4px;
`;

export const SettingLabel = styled.span`
  color: ${({ theme }) => theme.colors.grayScale["05"] || "#8A8D92"};
...
export const SettingValue = styled.span`
  color: ${({ theme }) => theme.colors.grayScale["00"] || "#0A0A0A"};
  font-family: Pretendard, sans-serif;
  font-size: 20px;
  font-style: normal;
  font-weight: 700;
